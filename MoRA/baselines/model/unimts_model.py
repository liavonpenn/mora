...
# REF: https://github.com/xiyuanzh/UniMTS/blob/main/contrastive.py
...
class ST_GCN_18(nn.Module):
    def __init__(self,
        ...
        self.st_gcn_networks = nn.ModuleList((
            ...
            st_gcn_block(256, 512, kernel_size, 1),
        ))
...
class UnimtsBackbone(nn.Module):
    def __init__(self):
        super().__init__()
    ...
    def encode_imu(self, imu_data):
        """Encode IMU data"""
        return self.model.acc(imu_data.float()).squeeze(-1).squeeze(-1)
    ...
    def forward(self, imu_data, text_data, masked_pos=None):
...
class UnimtsModel(nn.Module):
    def __init__(self, num_classes, feature_dim=512, threshold=0.5):
        super().__init__()
        self.backbone = UnimtsBackbone()
        self.head = UnimtsHead(num_classes)
    ...
    def forward(self, imu_data, return_features=False, training=True):

        features = self.backbone.encode_imu(imu_data)
        # features = self.full(features)

        if return_features:
            return features#.unsqueeze(1)
        
        logits_imu = self.head(features)

        return features, logits_imu

    def pretrain(self, x, text):
        """Pre-training method"""
        logits_imu, logits_text = self.backbone(x, text)  # B x M x hidden_dim
        return logits_imu, logits_text    